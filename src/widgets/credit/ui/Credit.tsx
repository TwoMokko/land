...
export function Credit({ idSection }: { idSection: string }) {
	...
	const imageSrc = !isReady
		...
		: isMobile
			? "/images/credit/credit-bg-mob.webp"
			: "/images/credit/credit-bg.webp";

	return (
		<section id={idSection} className={styles.credit}>
			<div className={styles.imgWrap}>
				<Image src={imageSrc} alt="credit-bg" fill className={styles.image} />
			</div>
			<div className={styles.info}>
				<div>
					<h2 className={styles.title}>
						{" "}
						Поможем подобрать <br /> лучшее кредитное <br /> предложение{" "}
					</h2>
					<ul className={styles.list}>
						<li>Ежемесячный платеж от 13 701 ₽/мес ⁴</li>
						<li>
							Кредит от 0,01%* <span>(ПСК от 0,01% до 10,5%)</span>
						</li>
					</ul>
				</div>
